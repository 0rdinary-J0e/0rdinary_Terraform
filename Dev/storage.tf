...
resource "aws_s3_bucket_ownership_controls" "ordinaryjoe_app_bucket_ownership_controls" {
  bucket = aws_s3_bucket.ordinaryjoe_app_bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          "${aws_s3_bucket.ordinaryjoe_app_bucket.arn}",
          "${aws_s3_bucket.ordinaryjoe_app_bucket.arn}/*"
        ]
      }
    ]
...
resource "aws_s3_bucket_policy" "ordinaryjoe_app_bucket_policy" {
  bucket = aws_s3_bucket.ordinaryjoe_app_bucket.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Allow CloudTrail to write logs to this bucket
      {
        ...
        Action    = "s3:PutObject",
        Resource  = "${aws_s3_bucket.ordinaryjoe_app_bucket.arn}/AWSLogs/${data.aws_caller_identity.current.account_id}/*",
        Condition = {
          StringEquals = {
...
resource "aws_ebs_volume" "ordinaryjoe-app_web_ebs" {
