...
resource "aws_db_instance" "ordinaryjoe_app_db" {
  ...
  db_subnet_group_name    = aws_db_subnet_group.ordinaryjoe_app_subnet_group.name
  vpc_security_group_ids  = [aws_security_group.ordinaryjoe_app_db_sg.id]  # Reference to the security group for RDS

  skip_final_snapshot = true

  tags = {
    Name = "ordinaryjoe_app_db"
  }
}
