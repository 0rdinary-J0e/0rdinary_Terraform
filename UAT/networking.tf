...
#   for_each = {
#     for dvo in aws_acm_certificate.ordinaryjoe_app_cert.domain_validation_options : dvo.domain_name => {
...
# resource "aws_acm_certificate_validation" "ordinaryjoe_app_cert_validation" {
#   certificate_arn         = aws_acm_certificate.ordinaryjoe_app_cert.arn
#   validation_record_fqdns = [for record in aws_route53_record.cert_validation : record.fqdn]
# }

# Hosted Zone for the domain ordinaryjoe.com
resource "aws_route53_zone" "ordinaryjoe_zone" {
  name = "ordinaryjoe.com"
}
...
resource "aws_lb_listener" "ordinaryjoe_app_http_listener" {
  load_balancer_arn = aws_lb.ordinaryjoe_app_lb.arn
  port              = 80
  protocol          = "HTTP"
...
# resource "aws_lb_listener" "ordinaryjoe_app_https_listener" {
#   load_balancer_arn = aws_lb.ordinaryjoe_app_lb.arn
#   port              = 443
#   protocol          = "HTTPS"
...
resource "aws_route53_record" "ordinaryjoe_app_record" {
  zone_id = aws_route53_zone.ordinaryjoe_zone.zone_id
  name    = "ordinaryjoe.com"
  type    = "A"
  alias {
    name                   = aws_lb.ordinaryjoe_app_lb.dns_name
    zone_id                = aws_lb.ordinaryjoe_app_lb.zone_id
